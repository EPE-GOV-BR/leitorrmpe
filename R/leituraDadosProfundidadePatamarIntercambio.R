...
#' do Usuario do Modelo Estrategico de geracao hidrotermica a subsistemas equivalentes do Projeto NEWAVE
#'
#' @param pastaCaso caracter com localizacao dos arquivos NEWAVE.
#'
#' @return \code{df.dadosProfundidadePatamarIntercambio} data frame com dados de 
#' profundidade de patamares de intercambio
#' \itemize{
#' \item origem do sistema (numero do subsistema/submercado A) 
#' (\code{$codSubsistemaOrigem})
#' \item destino do sistema (numero do subsistema/submercado B) 
#' (\code{$codSubsistemaDestino})
#' \item numero do patamar (\code{$patamar})
#' \item ano e mes referente a informacao (\code{$anoMes})
#' \item profundidade de patamar de intercambio 
#' (\code{$profundidadeIntercambio})
#' }
#'
#' @examples
#' \dontrun{
...
leituraDadosProfundidadePatamarIntercambio <- function(pastaCaso) {
  ...
  dadosPatamarIntercambio <- readr::read_lines(stringi::stri_enc_toutf8(paste(pastaCaso, arquivo, sep = "/")), 
  ...
  # Ajusta as linhas das colunas "ano", "codSubsistemaDestino" e 
  # "codSubsistemaOrigem", mantendo apenas os valores originais
  ...
  if (length(dplyr::filter(df.dadosProfundidadePatamarIntercambio, !is.na(ano))$ano) == 0) {
    df.dadosProfundidadePatamarIntercambio <- leituraLimiteInterligacoes(diretorio) %>%
      ...
      dplyr::select(codSubsistemaOrigem, 
                    codSubsistemaDestino, 
                    patamar, 
                    anoMes, 
                    profundidadeIntercambio)
  } else {
    df.dadosProfundidadePatamarIntercambio <- df.dadosProfundidadePatamarIntercambio %>%
      dplyr::mutate(
    ...
    # tranforma colunas em linhas, exceto as colunas "ano","codSubsistemaOrigem",
    # "codSubsistemaDestino" e "patamar".
    # o objetivo  criar duas colunas com as informacoes do "mes" e 
    # "profundidadeIntercambio"
