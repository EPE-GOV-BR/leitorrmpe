...
leituraLimiteInterligacoes <- function(pastaCaso){
  ...
  df.limiteInterligacoes <- df.limiteInterligacoes %>% 
    dplyr::mutate(codSubsistemaOrigem = ifelse(!is.na(DEZ), NA , codSubsistemaOrigem), 
    ...
    dplyr::mutate(codSubsistemaOrigem.i = zoo::na.locf(codSubsistemaOrigem.i), 
                  codSubsistemaDestino.i = zoo::na.locf(codSubsistemaDestino.i)) %>% 
    dplyr::mutate(codSubsistemaOrigem = ifelse(is.na(ano) & is.na(codSubsistemaOrigem), codSubsistemaOrigem.i, codSubsistemaOrigem), 
                  codSubsistemaDestino = ifelse(is.na(ano) & is.na(codSubsistemaDestino), codSubsistemaDestino.i, codSubsistemaDestino)) %>% 
    dplyr::mutate(codSubsistemaOrigem = zoo::na.locf(codSubsistemaOrigem), codSubsistemaDestino = zoo::na.locf(codSubsistemaDestino)) %>% dplyr::filter(!is.na(DEZ)) %>% 
