...
leituraMercadoEnergia <- function(pastaCaso){
  ...
  arquivoSistema <- leituraArquivos(pastaCaso) %>% dplyr::slice(2) %>% dplyr::pull(arquivo)
  ...
  arquivoDadosGerais <- leituraArquivos(pastaCaso) %>% dplyr::slice(1) %>% dplyr::pull(arquivo)
  ...
  if(flagConsideraCargaAdicional == 1){
    ...
    arquivoCadic <- leituraArquivos(pastaCaso) %>% dplyr::slice(23) %>% dplyr::pull(arquivo)
  ...
  filtroSub <- readr::read_fwf(I(dadosMercadoEnergiaTXT),
                               col_positions = readr::fwf_positions(1, 4, "classe"),
                               col_types = "c", skip_empty_rows = F) %>%
    tidyr::fill(everything()) %>% dplyr::pull() %>% as.numeric()
  
  anos <- filtroSub[!filtroSub %in% leituraSubsistema]
  quantosAnos <- anos[anos %in% (anos[duplicated(anos)]%>% unique())] %>% unique() %>% length()
