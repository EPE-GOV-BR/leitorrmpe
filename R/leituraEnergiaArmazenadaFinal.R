...
#' por REE (earmfXXX.out) e recupera esses valores por ano, mes e serie.
...
#'
#' @return \code{df.energiaArmazenadaFinal} data frame com os valores de energia 
#' armazenada final por REE
#' \itemize{
...
#' leituraEnergiaArmazenadaFinal("C:/PDE2027_Caso080", paralelo = TRUE)
#' }
#'
#' @export
leituraEnergiaArmazenadaFinal <- function(pasta, paralelo = FALSE) {
  if (missing(pasta)) {
  ...
  # para todos os anos
  df.energiaArmazenadaFinal <- tidyr::tibble(codREE = numeric(), 
                                             serie = numeric(), 
                                             anoMes = numeric(), 
                                             earmf = numeric())
  ...
  df.energiaArmazenadaFinal <- furrr::future_map_dfr(arquivos, function(arquivo) {
    ...
    inicioREE <- stringr::str_locate(arquivo, "earmf") %>%
      {
        .[1, 2] + 1
      } %>%
      unname()
    ...
    furrr::future_map_dfr(1:length(anos), function(andaAnos) {
      ...
      df.energiaArmazenadaFinal <- rbind(df.energiaArmazenadaFinal, 
                                         df.energiaArmazenadaFinalAno)
    })
  })
