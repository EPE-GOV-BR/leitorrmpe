...
leituraConfiguracaoTermoeletrica <- function(pastaCaso){
  ...
  arquivo <- leituraArquivos(pastaCaso) %>% dplyr::slice(5) %>% dplyr::pull(arquivo)
  ...
  df.configuracaoTermoeletrica <- data.frame(codUsinaTermica = as.numeric(stringr::str_sub(conft_dados[3:length(conft_dados)], 2, 5)),
