...
#'
#' Faz a leitura do arquivo do NEWAVE com dados de configuracao termoeletrica 
#' (conft.*).
...
#'
#' @return \code{df.configuracaoTermoeletrica} data frame com dados de 
#' configuracao termoeletrica
#' \itemize{
#' \item numero da usina termica (\code{$codUsinaTermica})
#' \item nome da usina (\code{$nomeUsina})
#' \item numero do subsistema/submercado a que pertence a usina 
#' (\code{$codSubsistema})
#' \item indicador de usina termica existente 
#' (EX = usina existente; EE = existente, com expansao; NE = nao existente, 
#' com expansao; NC = usina nao considerada) (\code{$status})
...
leituraConfiguracaoTermoeletrica <- function(pastaCaso) {
  ...
  # encontra o nome do arquivo com dados de usinas termoeletricas (conft.*) de 
