#' Leitor de dados de geracao termica total (Sistema Interligado Nacional)
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de geracao termica total do sistema interligado nacional (gttotsin.*) e recupera esses valores por ano, mes, patamar e serie.
...
#'
#' @param pasta localizacao dos arquivos do NEWAVE com dados de geracao termica total do sistema interligado nacional
#'
#' @return \code{df.geracaoTermicaTotalSin} data frame com os valores de geracao termica total - SIN
#' \itemize{
...
#' @examples
#' \dontrun{
#' leituraGeracaoTermicaTotalSin("C:/PDE2027_Caso080")}
#'
#' @export
leituraGeracaoTermicaTotalSin <- function(pasta) {
  if (missing(pasta)) {
    stop("favor indicar a pasta com os arquivos do NEWAVE")
  }
  
  # cria data frame de base para armazenar os dados de geracao termica total para todos os anos
  df.geracaoTermicaTotalSin <- tidyr::tibble()
  
  # seleciona somente os arquivos gttotsin
  arquivo <- list.files(pasta, pattern = "^gttotsin")
  if (length(arquivo) != 1) {
    stop(paste0("N\u00E3o foi encontrado o arquivo gttotsin.out em ", pasta, " ou existe mais de um arquivo."))
  }
  ...
  df.geracaoTermicaTotalSin <- purrr::map_df(1:length(anos), function(andaAnos) {
    ...
    numeroPatamar <- df.geracaoTermicaAnual %>% dplyr::distinct(patamar) %>% dplyr::pull() %>% max()
    ...
    df.geracaoTermicaAnual <- df.geracaoTermicaAnual %>%
      ...
      dplyr::select(serie, patamar, anoMes, geracao)
    # concatena dados num data frame unico
    df.geracaoTermicaTotalSin <- rbind(df.geracaoTermicaTotalSin, df.geracaoTermicaAnual)
  })
  return(df.geracaoTermicaTotalSin)
}
