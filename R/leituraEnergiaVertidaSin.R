#' Leitor de dados de energia vertida para o Sistema Interligado Nacional
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de energia vertida para o Sistema Interligado Nacional  (evertsin.*) e recupera esses valores por ano, mes e serie.
...
#'
#' @param pasta localizacao dos arquivos do NEWAVE com dados de energia vertida SIN
#'
#' @return \code{df.energiaVertidaSin} data frame com os valores de energia vertida para o SIN
#' \itemize{
...
leituraEnergiaVertidaSin <- function(pasta) {
  if (missing(pasta)) {
    stop("favor indicar a pasta com os arquivos do NEWAVE")
  }

  # cria data frame de base
  df.energiaVertidaSin <- tidyr::tibble(codSubmercado = numeric(), serie = numeric(), anoMes = numeric(), vertimento = numeric())

  # seleciona somente o arquivo evertsin
  arquivo <- list.files(pasta, pattern = "^evertsin")
  if (length(arquivo) != 1) {
    stop(paste0("N\u00E3o foi encontrado o arquivo evertsin.out em ", pasta, " ou existe mais de um arquivo."))
  }
  ...
  df.energiaVertidaSin <- purrr::map_df(1:length(anos), function(andaAnos) {
    ...
    df.energiaVertidaSin <- rbind(df.energiaVertidaSin, df.energiaVertidaAnual)
  })
  return(df.energiaVertidaSin)
}
