...
#'
#' Faz a leitura do arquivo do NEWAVE com dados de expansao termoeletrica 
#' (expt.*).
...
#'
#' @return \code{df.ExpansaoTermoeletrica} data frame com dados de usinas 
#' termoeletricas
#' \itemize{
...
leituraExpansaoTermoeletrica <- function(pastaCaso) {
  ...
  # encontra o nome do arquivo com dados de usinas termoeletricas (expt*) de 
  ...
  expt_dados <- readr::read_lines(stringi::stri_enc_toutf8(paste(pastaCaso, arquivo, sep = "/")), 
                                  locale = readr::locale(encoding = "latin1"))
  # cria a estrutura de dados contendo as informacoes sobre alteracoes dos 
  # parametros das usinas termicas
  df.ExpansaoTermoeletrica <- data.frame(
    codUsinaTermica = as.numeric(stringr::str_sub(expt_dados[3:length(expt_dados)], 1, 4)),
  ...
  if (nrow(dplyr::filter(df.ExpansaoTermoeletrica, !is.na(codUsinaTermica))) == 0) {
    df.ExpansaoTermoeletrica <- data.frame(codUsinaTermica = numeric(), 
                                           anoMes = numeric(), 
                                           capacidaInstalada = numeric(), 
                                           FCMaximo = numeric(), 
                                           TEIF = numeric(), IP = numeric(), 
                                           GTMin = numeric())
  } else {
    ...
    df.ExpansaoTermoeletrica <- df.ExpansaoTermoeletrica %>%
      dplyr::mutate(anoSeg = ifelse((dplyr::lead(codUsinaTermica) == codUsinaTermica) & (dplyr::lead(parametros) == parametros), 
                                    dplyr::lead(anoi), 
                                    NA))

    # ajusta campos "mf-mes final" e "anof-ano final", substiutindo valores NA 
    # pelo ulimto periodo do horizonte de simulacao ou pelo ano inicial - 1 da 
    # expansao seguinte, se houver
    # cria colunas de data inicial (datainci) e data final (datafim) no formato 
    # AAAAMM
    # mescla o dataframe anterior com a estrutura que define o periodo de 
    # simulacao (anoMes - AAAAMM)
    # avalia se o valor do CVU COnjuntural e valido para cada mes.
    # Se a um determinado mes for (maior ou igual) a data inicial e (menor ou 
    # igual) a data final, o valor fica vigente
    # filtro dos valores vigentes, considerando apenas aqueles diferentes de "nao"
    df.ExpansaoTermoeletrica <- dplyr::mutate(df.ExpansaoTermoeletrica,
      mf = ifelse(is.na(mf), 12, mf),
      ...
      dplyr::mutate(datainc = as.numeric(anoi * 100 + mi), 
                    datafim = as.numeric(anof * 100 + mf), 
                    aux = 1) %>%
      dplyr::filter(!is.na(codUsinaTermica)) %>%
      dplyr::select(-c("anoi", "anof", "mi", "mf", "anoSeg")) %>%
      dplyr::inner_join(dplyr::mutate(definePeriodo(pastaCaso), aux = 1), 
                        by = "aux", 
                        relationship = "many-to-many") %>%
      dplyr::select(-c("aux")) %>%
      dplyr::mutate(validadeExpt = ifelse(((anoMes >= datainc) & (anoMes <= datafim)), 
                                          "sim", 
                                          "nao")) %>%
    ...
    if (("TEIFT" %in% colnames(df.ExpansaoTermoeletrica)) == FALSE) {
      df.ExpansaoTermoeletrica <- dplyr::mutate(df.ExpansaoTermoeletrica, TEIFT = NA)
    }
    if (("IPTER" %in% colnames(df.ExpansaoTermoeletrica)) == FALSE) {
      df.ExpansaoTermoeletrica <- dplyr::mutate(df.ExpansaoTermoeletrica, IPTER = NA)
    }
    if (("GTMIN" %in% colnames(df.ExpansaoTermoeletrica)) == FALSE) {
      df.ExpansaoTermoeletrica <- dplyr::mutate(df.ExpansaoTermoeletrica, GTMIN = NA)
    }
    if (("FCMAX" %in% colnames(df.ExpansaoTermoeletrica)) == FALSE) {
      df.ExpansaoTermoeletrica <- dplyr::mutate(df.ExpansaoTermoeletrica, FCMAX = NA)
    }

    df.ExpansaoTermoeletrica <- dplyr::select(df.ExpansaoTermoeletrica, 
                                              codUsinaTermica, 
                                              anoMes, 
                                              POTEF, 
                                              FCMAX, 
