...
#'
#' @return \code{df.ClassesTermicas} data frame com dados de usinas 
#' termeletricas
#' \itemize{
...
leituraClassesTermicas <- function(pastaCaso) {
  ...
  # encontra o nome do arquivo com dados de usinas termoeletricas (clast.*) de 
  ...
  clast_dados <- readr::read_lines(stringi::stri_enc_toutf8(paste(pastaCaso, arquivo, sep = "/")), 
                                   locale = readr::locale(encoding = "latin1"), n_max = 500)
  ...
  df.ClassesTermicas <- data.frame(
    codUsinaTermica = as.integer(stringr::str_sub(clast_estrutural, 2, 5)),
    nomeUsina = stringr::str_sub(clast_estrutural, 7, 18),
    tipoComb = stringr::str_sub(clast_estrutural, 20, 29)
  )

  # cria a estrutura de dados contendo as informacoes do CUsto Variavel Unitario 
  # (CVU) ao longo dos anos do horizonte de simulacao
  ...
  # tranforma colunas "Anos" em linhas, com a coluna "CVU" definindo o valor do 
  # CVU para cada ano
  # mescla o dataframe anterior com a estrutura que define o periodo de 
  # simulacao (anoMes - AAAAMM)
  df.ClassesTermicas <- dplyr::inner_join(df.ClassesTermicas, 
                                          df.ClassesTermicasSub, 
                                          by = "codUsinaTermica", 
                                          relationship = "many-to-many") %>%
    tidyr::pivot_longer(-c(1, 2, 3), names_to = "ano", values_to = "CVU") %>%
    dplyr::mutate(ano = as.numeric(ano)) %>%
    dplyr::select(-c("nomeUsina")) %>%
    dplyr::inner_join(definePeriodo(pastaCaso), 
                      by = "ano", 
                      relationship = "many-to-many") %>%
    dplyr::filter(!is.na(codUsinaTermica)) %>%
    dplyr::select(-c("ano", "mes"))

  # se a variavel "inicioClast" for maior que 1, existe um CVU Conjuntural que 
  ...
  if (length(inicioClast) > 1) {
    ...
    df.ClassesTermicasConj <- data.frame(
      codUsinaTermica = as.integer(stringr::str_sub(clast_conjuntural, 2, 5)),
    ...
    # pelo ulimto periodo do horizonte de simulacao
    ...
    # se a um determinado mes for (maior ou igual) a data inicial e (menor ou igual) 
    # a data final, o valor fica vigente
    # filtro dos valores vigentes, considerando apenas aqueles diferentes de "nao"
    df.ClassesTermicasConj <- dplyr::mutate(df.ClassesTermicasConj, 
      ...
      dplyr::mutate(datainc = as.numeric(anoi * 100 + mi), 
                    ...
                    aux = 1) %>%
      dplyr::filter(!is.na(codUsinaTermica)) %>%
      dplyr::select(-c("anoi", "anof", "mi", "mf")) %>%
      dplyr::inner_join(dplyr::mutate(definePeriodo(pastaCaso), aux = 1), 
                        by = "aux", 
                        relationship = "many-to-many") %>%
      dplyr::select(-c("aux")) %>%
      dplyr::mutate(validadeCVU = ifelse(((anoMes >= datainc) & (anoMes <= datafim)), 
                                         "sim", 
                                         "nao")) %>%
      dplyr::filter(validadeCVU != "nao") %>%
      dplyr::select(-c("validadeCVU", "datainc", "datafim", "ano", "mes"))

    # mescla as informacoes do CVU estrutural com o CVU COnjuntural, substituindo 
    # o primeiro pelo ultimo, caso esse nao seja nulo

    df.ClassesTermicas <- dplyr::left_join(df.ClassesTermicas, 
                                           df.ClassesTermicasConj, 
                                           by = c("codUsinaTermica", "anoMes")) %>%
      dplyr::mutate(CVU = ifelse(is.na(valornew), CVU, valornew)) %>%
      dplyr::select(codUsinaTermica, anoMes, tipoComb, CVU)
  }
