...
leituraArquivos <- function(pastaCaso) {
  ...
  df.arquivos <- readr::read_delim(stringi::stri_enc_toutf8(paste(pastaCaso, arquivo, sep = "/")), delim = ":",
  ...
  df.arquivos <- df.arquivos %>% dplyr::mutate(descricao = stringr::str_squish(toupper(descricao)),
