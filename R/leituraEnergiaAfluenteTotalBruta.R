...
#' bruta por REE (eafbXXX.out) e recupera esses valores por ano, mes e serie.
...
#' energia afluente total bruta por REE
#' \itemize{
...
#' leituraEnergiaAfluenteTotalBruta("C:/PDE2027_Caso080", paralelo = TRUE)
#' }
#'
#' @export
leituraEnergiaAfluenteTotalBruta <- function(pasta, paralelo = FALSE) {
  if (missing(pasta)) {
  ...
  # para todos os anos
  df.energiaAfluenteTotalBruta <- tidyr::tibble(codREE = numeric(), 
                                                serie = numeric(), 
                                                anoMes = numeric(), 
                                                eafb = numeric())
