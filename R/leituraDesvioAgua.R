...
leituraDesvioAgua <- function(pastaCaso){
  ...
  arquivo_dsvagua <- leituraArquivos(pastaCaso) %>% dplyr::slice(28) %>% dplyr::pull(arquivo)
  ...
  df.DesvioAgua <- readr::read_fwf(I(dadosDesvioAguaTXT),
                                   ...
                                   col_types = "iiddddddddddddic") %>% dplyr::mutate(comentario = zoo::na.locf(stringr::str_trim(comentario))) # tira espacos do comentario e preenche linhas com comentario NA
  ...
  df.DesvioAgua <-  tidyr::pivot_longer(df.DesvioAgua, -c("ano", "codUsina", "flagDesvio", "comentario"), 
    ...
    dplyr::mutate(desvioUsoConsuntivo = ifelse(stringr::str_detect(comentario, "^Usos_C"), valDesvio, 0.0),
    ...
    dplyr::group_by(codUsina, anoMes) %>% dplyr::summarise(flagDesvio = mean(flagDesvio),
