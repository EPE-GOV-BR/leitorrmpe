...
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de cota de montante por usina  
#' (hmontxxx.*) e recupera esses valores por codigo da usina, ano, mes e serie.
...
#'
#' @param pasta localizacao dos arquivos do NWLISTOP com dados de cota de 
#' montante por usina
#' @param paralelo booleano que indica se a leitura dos arquivos sera realizada
#' de forma serial ou em paralelo (Default: FASLE)
#'
#' @return \code{df.CotaMontanteUsina} data frame com os valores de cota de  
#' montante por usina
#' \itemize{
...
#' leituraCotaMontanteUsina("C:/PDE2027_Caso080", paralelo = TRUE)
#' }
#'
#' @export
leituraCotaMontanteUsina <- function(pasta, paralelo = FALSE) {
  if (missing(pasta)) {
  ...
  df.CotaMontanteUsina <- furrr::future_map_dfr(arquivos, function(arquivo) {
    ...
    furrr::future_map_dfr(1:length(anos), function(andaAnos) {
      ...
      # acontece na numeracao das series historicas. Assim troca-se o numero 
      # ou ano pelo valor dentro de uma sequencia para cada ano.
