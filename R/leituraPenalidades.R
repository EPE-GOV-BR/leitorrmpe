...
#' \itemize{
#' \item tipo de penalidade: 1 para penalidade aplicada a utilizacao da variavel 
#' de folga (PENALID1); 2 para penalidade aplicada a utilizacao da variavel de 
#' folga no segundo patamar (PENALID2) (\code{$tipoPenalidade})
#' \item numero do REE (\code{$codREE})
#' \item numero do patamar de carga (\code{$patamar})
#' \item penalidade para variavel de folga relativa a outros usos da agua 
#' (\code{$desvio})
#' \item penalidade para variavel de folga relativa a intercambio minimo 
#' (\code{$intercambioMinimo})
#' \item penalidade para variavel de folga relativa a restricao de defluencia 
#' minima obrigatoria (\code{$defluenciaMinima})
#' \item penalidade para variavel de folga relativa a restricao de armazenamento 
#' minimo obrigatorio (\code{$volumeMinimo})
#' \item penalidade para variavel de folga relativa a geracao hidraulica minima 
#' (\code{$geracaoMinima})
#'
#' }
#'
...
leituraPenalidades <- function(pastaCaso) {
  ...
  # encontra o nome do arquivo com dados de mercado (penalid.*) de acordo com a 
  ...
  df.ree <- leituraREE(pastaCaso) %>%
    dplyr::mutate(aux = 1) %>%
    dplyr::select(codREE, aux)
  # cria dataframe expandindo codREE para chaves sem codREE
  df.chaves <- df.penalidades[is.na(df.penalidades$codREE), ] %>%
    dplyr::select(-codREE) %>%
    dplyr::mutate(aux = 1)
  df.chaves <- dplyr::inner_join(df.chaves, df.ree, by = "aux") %>% 
    dplyr::select(-aux)


  df.penalidades <- rbind(df.penalidades, df.chaves)
  df.penalidades <- df.penalidades[!is.na(df.penalidades$codREE), ] %>% 
    dplyr::arrange(codREE)
  ...
  df.penalidade1 <- df.penalidade1 %>%
    tidyr::pivot_wider(names_from = chave, values_from = penalidade1) %>%
    dplyr::mutate(tipoPenalidade = 1)
  df.penalidade2 <- df.penalidade2 %>%
    tidyr::pivot_wider(names_from = chave, values_from = penalidade2) %>%
    dplyr::mutate(tipoPenalidade = 2)
  ...
  df.penalidades <- df.penalidades %>% dplyr::select(
