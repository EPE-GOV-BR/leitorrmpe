...
#'
#' Faz o calculo da media e do desvio padrao dos CMOs por submercado e por ano 
#' de estudo,reproduzindo o calculo que e encontrado ao final do relatorio de 
#' saida do NEWAVE
#'
#' @param pasta localizacao dos arquivos do NEWAVE para os quais deseja efetuar 
#' o calculo
#'
...
calculoMediaDpCMO <- function(pasta) {
  ...
  df.cmoMedio <- dplyr::left_join(df.cmo, df.duracao, by = c("patamar", "anoMes")) %>%
    ...
    dplyr::summarise(mediaCMO = round(mean(cmoMedio), 2), 
                     dpCMO = round(stats::sd(cmoMedio) / (sqrt(seriesHidro - 1)), 2)
                     ) %>%
    dplyr::mutate(ano = as.character(ano))

  df.cmoMedioPeriodoPlanej <- dplyr::left_join(df.cmo, df.duracao, 
                                               by = c("patamar", "anoMes")) %>%
    dplyr::mutate(cmoMedio = custoMarginalDemanda * duracaoPatamar) %>%
    dplyr::group_by(codSubmercado, serie, anoMes) %>%
    dplyr::summarise(cmoMedio = sum(cmoMedio)) %>%
