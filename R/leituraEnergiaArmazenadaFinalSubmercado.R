...
#' por Submercado (earmfMXXX.out) e recupera esses valores por ano, mes e serie.
...
#'
#' @param pasta localizacao dos arquivos do NEWAVE com dados de energia 
#' armazenada final por submercado
#'
#' @return \code{df.energiArmazenadaFinalSubmercadoSubmercado} data frame com os 
#' valores de energia armazenada final por REE
#' \itemize{
...
leituraEnergiaArmazenadaFinalSubmercado <- function(pasta) {
  ...
  # para todos os anos
  df.energiArmazenadaFinalSubmercado <- tidyr::tibble(codSubmercado = numeric(), 
                                                      serie = numeric(), 
                                                      anoMes = numeric(), 
                                                      earmf = numeric())
  ...
  df.energiArmazenadaFinalSubmercado <- purrr::map_df(arquivos, function(arquivo) {
    ...
    inicioREE <- stringr::str_locate(arquivo, "earmfm") %>%
      ...
      unname()
    codSubmercado <- stringr::str_sub(arquivo, inicioREE, inicioREE + 2) %>% 
      as.integer()
    ...
    purrr::map_df(1:length(anos), function(andaAnos) {
      ...
      df.energiArmazenadaFinalSubmercado <- rbind(df.energiArmazenadaFinalSubmercado, 
                                                  df.energiArmazenadaFinalSubmercadoAno)
    })
  })
