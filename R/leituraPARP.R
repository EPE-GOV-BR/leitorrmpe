...
leituraPARP <- function(pasta) {
  ...
  # encontra os anos
  nomeREE_configuracao <- dadosBrutos[which(stringr::str_detect(dadosBrutos, "SERIE  DE ENERGIAS DO REE"))] %>% stringr::str_remove("SERIE  DE ENERGIAS DO REE") %>% stringr::str_remove("CONFIGURACAO No.")
  ...
  lista <- crossmap::map_vec(1:length(nomeREE), function(andaREE) {
    ...
    lista <- list(df.ordemOriginal = df.ordemOriginal,
  ...
  lista <- list(df.ordemOriginal = purrr::map_df(lista, ~.[[1]]), 
