#' Leitor de dados de vertimento fio dagua turbinavel por REE
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de vertimento fio dagua turbinavel por REE (verturbxxx.*) e recupera esses valores por ano, mes e serie.
...
#'
#' @return \code{df.vertimentoFioDaguaTurbinavel} data frame com os valores de vertimento fio dagua turbinavel
#' \itemize{
#' \item codigo do REE (\code{$codREE})
#' \item serie (\code{$serie})
#' \item valor de ano e mes (\code{$anoMes})
#' \item vertimento fio dagua turbinavel [MWmes] (\code{$vertimento})
#' }
#'
#' @examples
#' \dontrun{
#' leituraVertimentoFioDaguaTurbinavel("C:/PDE2027_Caso080")}
#'
#' @export

leituraVertimentoFioDaguaTurbinavel <- function(pasta) {
  if (missing(pasta)) {
    stop("favor indicar a pasta com os arquivos do NEWAVE")
  }
  
  # cria data frame de base 
  df.vertimentoFioDaguaTurbinavel <- tidyr::tibble(codREE = numeric(), serie = numeric(), anoMes = numeric(), vertimento = numeric())
  
  # seleciona somente os arquivos verturb
  arquivos <-  setdiff(list.files(pasta, pattern = "^verturb"), list.files(pasta, pattern = "^verturb[pms]"))
  if (length(arquivos) == 0) {
    stop(paste0("N\u00E3o foram encontrados os arquivos verturbXXX.out em ", pasta))
  }
  
  df.vertimentoFioDaguaTurbinavel <- purrr::map_df(arquivos, function(arquivo) {
    ...
    inicioREE <- stringr::str_locate(arquivo, "verturb") %>% {.[1,2] + 1} %>% unname()
    ...
    purrr::map_df(1:length(anos), function(andaAnos) {
      ...
      df.vertimentoFioDaguaTurbinavelAnual <- df.vertimentoFioDaguaTurbinavelAnual %>%
        ...
        dplyr::select(codREE, serie, anoMes, vertimento)
      # concatena dados num data frame unico
      df.vertimentoFioDaguaTurbinavel <- rbind(df.vertimentoFioDaguaTurbinavel, df.vertimentoFioDaguaTurbinavelAnual)
    })
  })
  return(df.vertimentoFioDaguaTurbinavel)
}
