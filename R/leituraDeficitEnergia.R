...
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de deficit de energia por 
#' subsistema e por deficit no patamar  (defXXXpXXX.*) e recupera esses valores 
#' por defcit, ano, mes, patamar e serie.
...
#'
#' @param pasta localizacao dos arquivos do NWLISTOP com dados de deficit de 
#' energia
#' @param paralelo booleano que indica se a leitura dos arquivos sera realizada
#' de forma serial ou em paralelo (Default: FASLE)
#'
#' @return \code{df.deficitEnergia} data frame com os valores de deficit de 
#' energia
#' \itemize{
...
#' leituraDeficitEnergia("C:/PDE2027_Caso080", paralelo = TRUE)
#' }
#'
#' @export
leituraDeficitEnergia <- function(pasta, paralelo = FALSE) {
  if (missing(pasta)) {
  ...
  df.deficitEnergia <- furrr::future_map_dfr(arquivos, function(arquivo) {
    ...
    furrr::future_map_dfr(1:length(anos), function(andaAnos) {
      ...
      numeroPatamar <- df.deficitEnergiaAnual %>%
        ...
        dplyr::pull() %>%
        max()
      series <- rep(1:(nrow(df.deficitEnergiaAnual) / numeroPatamar), 
                    each = numeroPatamar)
