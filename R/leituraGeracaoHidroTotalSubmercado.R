...
#' @examples
#' \dontrun{
#' leituraGeracaoHidroTotalSubmercado("C:/PDE2027_Caso080")}
#'
#' @export
leituraGeracaoHidroTotalSubmercado <- function(pasta) {
  if (missing(pasta)) {
    stop("favor indicar a pasta com os arquivos do NEWAVE")
  }
  
  # cria data frame de base
  df.geracaoHidroTotalSubmercado <- tidyr::tibble()
  ...
  df.geracaoHidroTotalSubmercado <- purrr::map_df(arquivos, function(arquivo) {
    ...
    inicioSubmercado <- stringr::str_locate(arquivo, "ghtotm") %>% {.[1,2] + 1} %>% unname()
    ...
    purrr::map_df(1:length(anos), function(andaAnos) {
      ...
      df.geracaoHidroTotalSubmercado <- rbind(df.geracaoHidroTotalSubmercado, df.geracaoHidroAnual)
    })
  })
  return(df.geracaoHidroTotalSubmercado)
}
