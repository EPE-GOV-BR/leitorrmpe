#' Leitor de dados de energia vertida por REE
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de energia vertida por REE 
#' (evertxxx.*) e recupera esses valores por ano, mes e serie.
...
#'
#' @return \code{df.energiaVertida} data frame com os valores de energia vertida
#' \itemize{
...
leituraEnergiaVertida <- function(pasta) {
  if (missing(pasta)) {
    stop("favor indicar a pasta com os arquivos do NEWAVE")
  }

  # cria data frame de base para armazenar os dados de energia vertida para 
  # todos os anos
  df.energiaVertida <- tidyr::tibble(codREE = numeric(), 
                                     serie = numeric(), 
                                     anoMes = numeric(), 
                                     vertimento = numeric())

  # seleciona somente os arquivos evert
  arquivos <- setdiff(list.files(pasta, pattern = "^evert"), list.files(pasta, pattern = "^evert[pms]"))
  if (length(arquivos) == 0) {
    stop(paste0("N\u00E3o foram encontrados os arquivos evertXXX.out em ", pasta))
  }

  df.energiaVertida <- purrr::map_df(arquivos, function(arquivo) {
    ...
    inicioREE <- stringr::str_locate(arquivo, "evert") %>%
      {
        .[1, 2] + 1
      } %>%
      unname()
    ...
    purrr::map_df(1:length(anos), function(andaAnos) {
      ...
      df.energiaVertida <- rbind(df.energiaVertida, df.energiaVertidaAnual)
    })
  })
  return(df.energiaVertida)
}
