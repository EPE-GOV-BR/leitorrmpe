...
leituraLimiteAgrupamentoInterligacoes <- function(pastaCaso) {
  ...
  suppressWarnings(
    df.limiteAgrupamentoInterligacoes <- data.frame(
      codAgrup = as.numeric(stringr::str_sub(dadosLimiteAgrintTXT, 2, 4)),
  ...
  df.limiteAgrupamentoInterligacoes <- df.limiteAgrupamentoInterligacoes %>%
    dplyr::mutate(
      dataini = as.numeric(ANO_INI) * 100 + as.numeric(MES_INI),
  ...
  df.limiteAgrupamentoInterligacoes <- dplyr::inner_join(dplyr::mutate(df.limiteAgrupamentoInterligacoes, aux = 1), dplyr::mutate(definePeriodo(pastaCaso), aux = 1), by = c("aux"), relationship = "many-to-many") %>%
    dplyr::select(-c("aux")) %>%
    dplyr::mutate(valexpt = ifelse(((anoMes >= dataini) & (anoMes <= datafim | is.na(datafim))), "sim", "nao")) %>%
    dplyr::filter(valexpt != "nao") %>%
    dplyr::select(-c("valexpt", "dataini", "datafim", "ano", "mes"))
  ...
  df.limiteAgrupamentoInterligacoes <- tidyr::pivot_longer(df.limiteAgrupamentoInterligacoes,
    ...
    dplyr::mutate(patamar = as.integer(patamar)) %>%
    dplyr::filter(!is.na(limiteAgrup))
  ...
  df.pat <- dplyr::inner_join(dplyr::mutate(dplyr::distinct(dplyr::select(leituraAgrupamentoInterligacoes(pastaCaso), "codAgrup")), aux = 1),
    data.frame(patamar = 1:nrow(dplyr::distinct(dplyr::select(leituraDadosDuracaoPatamar(pastaCaso), "patamar"))), aux = 1),
