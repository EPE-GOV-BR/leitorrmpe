...
leituraIntercambioEnergia <- function(pasta) {
  ...
  df.intercambioEnergia <- purrr::map_df(arquivos, function(arquivo) {
    ...
    inicioSubsistemaOrigem <- stringr::str_locate(arquivo, "int") %>% {.[1,2] + 1} %>% unname()
    ...
    inicioSubsistemaDestino <- stringr::str_locate(arquivo, "int") %>% {.[1,2] + 4} %>% unname()
    ...
    purrr::map_df(1:length(anos), function(andaAnos) {
      ...
      numeroPatamar <- df.intercambioEnergiaAnual %>% dplyr::distinct(patamar) %>% dplyr::pull() %>% max()
