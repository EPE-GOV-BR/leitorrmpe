#' Leitor de dados de geracao hidraulica total (Sistema Interligado Nacional)
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de geracao hidraulica total do sistema interligado nacional (ghtotsin.*) e recupera esses valores por ano, mes, patamar e serie.
...
#'
#' @param pasta localizacao dos arquivos do NEWAVE com dados de geracao hidroeletrica total do sistema interligado nacional
#'
#' @return \code{df.geracaoHidroTotalSin} data frame com os valores de geracao hidroeletrica total - SIN
#' \itemize{
...
#' @examples
#' \dontrun{
#' leituraGeracaoHidroTotalSin("C:/PDE2027_Caso080")}
#'
#' @export
leituraGeracaoHidroTotalSin <- function(pasta) {
  if (missing(pasta)) {
    stop("favor indicar a pasta com os arquivos do NEWAVE")
  }
  
  # cria data frame de base
  df.geracaoHidroTotalSin <- tidyr::tibble()
  
  # seleciona somente o arquivo ghtotsin
  arquivo <- list.files(pasta, pattern = "^ghtotsin")
  if (length(arquivo) != 1) {
  ...
  df.geracaoHidroTotalSin <- purrr::map_df(1:length(anos), function(andaAnos) {
    ...
    numeroPatamar <- df.geracaoHidroAnual %>% dplyr::distinct(patamar) %>% dplyr::pull() %>% max()
    ...
    df.geracaoHidroTotalSin <- rbind(df.geracaoHidroTotalSin, df.geracaoHidroAnual)
  })
  return(df.geracaoHidroTotalSin)
}
