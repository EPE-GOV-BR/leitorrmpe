  ...
  df.geracaoTermicaClassesTotal <- purrr::map_df(arquivos, function(arquivo) {
    ...
    if(any(filtro)){
      ...
      purrr::map_df(1:length(anos), function(andaAnos) {
        ...
        numeroPatamar <- df.geracaoTermicaClassesTotalAnual %>% dplyr::distinct(patamar) %>% dplyr::pull() %>% max()
        numeroSerie <-  df.geracaoTermicaClassesTotalAnual %>% dplyr::distinct(serie) %>% dplyr::pull() %>% length()
        numeroUsinas <- df.geracaoTermicaClassesTotalAnual %>% dplyr::distinct(codUsina) %>% dplyr::pull() %>% length()
