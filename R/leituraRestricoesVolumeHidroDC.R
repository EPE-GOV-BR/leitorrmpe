...
leituraRestricoesVolumeHidroDC <- function(pastaCaso) {
  ...
  df.HV <- data.frame(
    numRest = as.numeric(stringr::str_sub(registrosHV, 5, 7)),
  ...
  df.LV <- data.frame(
    numRest = as.numeric(stringr::str_sub(registrosLV, 5, 7)),
  ...
  df.CV <- data.frame(
    numRest = as.numeric(stringr::str_sub(registrosCV, 5, 7)),
