...
leituraRestricoesVazaoHidroDC <- function(pastaCaso) {
  ...
  df.HQ <- data.frame(numRest = as.numeric(stringr::str_sub(registrosHQ, 5, 7)),
  ...
  df.LQ <- data.frame(numRest = as.numeric(stringr::str_sub(registrosLQ, 5, 7)),
  ...
  df.CQ <- data.frame(numRest = as.numeric(stringr::str_sub(registrosCQ, 5, 7)),
