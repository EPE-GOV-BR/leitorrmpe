...
leituraManutencoesProgramadas <- function(pastaCaso) {
  ...
  if (nrow(df.dadosManutencao) > 0 && sum(!is.na(df.dadosManutencao$codUsinaTermica)) > 0) {
    ...
    df.horizonte <- data.frame(anoMes = seq(anoMesInicio, anoMesFim, 1 / 12)) %>%
      dplyr::mutate(
        diasMes = as.integer(zoo::as.Date(anoMes + 1 / 12) - zoo::as.Date(anoMes)),
        aux = 1
      )

    df.dadosManutencao <- df.dadosManutencao %>% dplyr::mutate(
      dataInicio = zoo::as.Date(dataInicio, "%d%m%Y"),
    ...
    df.dadosManutencao <- df.dadosManutencao %>%
      dplyr::mutate(
        entreMes = anoMes >= zoo::as.yearmon(dataInicio) & anoMes <= zoo::as.yearmon(dataFim),
