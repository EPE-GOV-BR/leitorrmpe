...
leituraSubmotorizacaoREE <- function(pastaCaso) {
  ...
  # encontra as REEs
  nomeREE <- submotorizacaoTXT[which(stringr::str_detect(submotorizacaoTXT, "REE:"))] %>% stringr::str_remove("REE:") %>% stringr::str_trim()
  ...
  for (andaRee in 1:length(nomeREE)) {
    ...
    df.submotorizacaoRee <- submotorizacaoTXT[inicioREE[andaRee]:fimREE[andaRee]] %>% 
      stringr::str_squish() %>% 
      I() %>% readr::read_delim(" ", show_col_types = FALSE) %>%
