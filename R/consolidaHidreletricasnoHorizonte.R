...
consolidaHidreletricasnoHorizonte <- function(pastaCaso){
  ...
  df.potef <- dplyr::inner_join(df.dadosConfiguracao, df.configuracaoHidro, by = c("codUsina")) %>% 
    ...
    dplyr::mutate(aux = 1) %>% dplyr::inner_join(dplyr::mutate(definePeriodo(pastaCaso), aux = 1), by = c("aux"), relationship = "many-to-many") %>% 
    ...
    dplyr::group_by(codUsina,anoMes) %>% dplyr::summarise(potenciaEfetivaUsina = sum(potenciaEfetivaConjunto), 
  ...
  df.consolidadoUHEnoHorizonte <- dplyr::inner_join(df.dadosUsinasHidroeletricas, df.configuracaoHidro, by = c("codUsina")) %>% 
    ...
    dplyr::mutate(volumeMaximoFinal = ifelse(!is.na(volumeMaximo.y) & idModificacaoUsina==1, volumeMaximo.y, volumeMaximo.x),
    ...
    dplyr::select(codUsina,nomeUsina,anoMes,codREE,canalFugaFinal,volumeMaximoFinal,volumeMinimoFinal,
