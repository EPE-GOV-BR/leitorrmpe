  ...
  if (length(dadosValidos) == 0) {
    ...
    df.registrosTipo1 <- readr::read_fwf(I(dadosValidos),
      col_positions = readr::fwf_positions(
        c(1, 6, 19, 22, 32, 38), # vetor com as posicoes iniciais de cada campo
      ...
      col_types = readr::cols(
        codUsina = readr::col_double(), nomeUsina = readr::col_character(), mesInicioEnchimentoVM = readr::col_integer(),
    ...
    df.registrosTipo1 <- df.registrosTipo1 %>%
      dplyr::mutate(
        mesInicioEnchimentoVM = ifelse(!is.na(nomeUsina) & is.na(mesInicioEnchimentoVM), 0, mesInicioEnchimentoVM),
        ...
        volumeMortoCheio = ifelse(!is.na(nomeUsina) & is.na(volumeMortoCheio), 0, volumeMortoCheio)
      ) %>%
    ...
    df.dadosExpansaoHidro <- readr::read_fwf(I(dadosValidos[dadosTempo]),
      col_positions = readr::fwf_positions(
        c(45, 48, 53, 61, 65), # vetor com as posicoes iniciais de cada campo
      ...
      col_types = readr::cols(
        mesEntradaOperacao = readr::col_integer(), anoEntradaOperacao = readr::col_integer(), potenciaUnitaria = readr::col_double(),
