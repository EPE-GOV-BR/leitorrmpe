...
#'
#' Faz a leitura dos arquivos do NEWAVE com dados de volume armazenado 
#' percentual por usina  (varmpuhxxx.*) e recupera esses valores por codigo da 
...
#'
#' @param pasta localizacao dos arquivos varmpuhxxx.* do NWLISTOP
#'
#' @return \code{df.VolArmPercentualPercentualUsina} data frame com os valores 
#' de volume armazenado percentual por usina
#' \itemize{
...
leituraVolArmPercentualUsina <- function(pasta) {
  ...
  df.VolArmPercentualUsina <- purrr::map_df(arquivos, function(arquivo) {
    ...
    inicioUsina <- stringr::str_locate(arquivo, "varmpuh") %>%
      ...
      unname()
    codUsina <- stringr::str_sub(arquivo, inicioUsina, inicioUsina + 2) %>% 
      as.integer()
