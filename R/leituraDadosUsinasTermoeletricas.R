...
#'
#' Faz a leitura do arquivo do NEWAVE com dados de usinas termoeletricas 
#' (term.*).
#' Usa como referencia para a leitura do arquivo as posicoes definidas no Manual 
#' do Usuario do Modelo Estrategico de geracao hidrotermica a subsistemas e
#' quivalentes do Projeto NEWAVE
#'
#' @param pastaCaso caracter com localizacao dos arquivos NEWAVE.
#'
#' @return \code{df.dadosUsinasTermoeletricas} data frame com dados de usinas 
#' termeletricas
#' \itemize{
...
leituraDadosUsinasTermoeletricas <- function(pastaCaso) {
  ...
  # encontra o nome do arquivo com dados de usinas termoeletricas (term.*) de 
  ...
  term_dados <- readr::read_lines(stringi::stri_enc_toutf8(paste(pastaCaso, arquivo, sep = "/")), 
                                  locale = readr::locale(encoding = "latin1"))

  # cria a estrutura de dados contendo as informacoes iniciais do arquivo term
  df.dadosUsinasTermoeletricas <- data.frame(
    codUsinaTermica = as.numeric(stringr::str_sub(term_dados[3:length(term_dados)], 2, 4)),
  ...
  # cria a estrutura de dados contendo as informacoes adicionais do arquivo 
  # term, referente a geracao minima
  ...
  df.dadosUsinasTermoeletricasSub <- tidyr::pivot_longer(df.dadosUsinasTermoeletricasSub, -1, 
                                                         names_to = "mes", 
                                                         values_to = "GTMin")
  ...
  # transforma todas as colunas (exceto a primeira) em linhas
  df.dadosUsinasTermoeletricasSubDemaisANos <- tidyr::pivot_longer(df.dadosUsinasTermoeletricasSubDemaisANos, 
                                                                   -c(1), 
                                                                   names_to = "parametros", 
                                                                   values_to = "GTMinDA")
  ...
  # tranforma colunas em linhas, com a coluna "parametros" definindo as 
  # informacoes de interesse das termicas
  # mescla as informacoes de Geracao Minima dos demais anos
  # reescreve o valor de Geracao Minima para os demais anos do horizonte de 
  # estudo, a partir do 2o ano
  # transforma linhas (coluna "parametros") em colunas
  df.dadosUsinasTermoeletricas <- dplyr::inner_join(df.dadosUsinasTermoeletricas, 
                                                    df.dadosUsinasTermoeletricasSub, 
                                                    by = c("codUsinaTermica" = "codUsinaTermica"), 
    ...
    tidyr::pivot_longer(-c(1, 7), 
                        names_to = "parametros", 
                        values_to = "valor") %>%
    dplyr::left_join(df.dadosUsinasTermoeletricasSubDemaisANos, 
                     by = c("codUsinaTermica", "parametros")) %>%
    dplyr::mutate(valor = ifelse((anoMes >= (((df.dadosGerais$anoInicio + 1) * 100 + 1))) & (parametros == "GTMin"), 
                                 GTMinDA, 
                                 valor)) %>%
    ...
    dplyr::ungroup() %>%
    tidyr::pivot_wider(names_from = "parametros", 
                       values_from = "valor") %>%
    dplyr::select(-row_id)
