...
leituraAlteracaoDadosUsinasHidro <- function(pastaCaso) {
  ...
  arquivo <- leituraArquivos(pastaCaso) %>% dplyr::slice(4) %>% dplyr::pull(arquivo)
  ...
  df.alteracaoHidro <- readr::read_fwf(paste(pastaCaso, arquivo, sep = "/"), 
                                       ...
                                       skip = 2)
  df.alteracaoHidro <- df.alteracaoHidro %>% dplyr::mutate(chave = toupper(chave), # garante compatibilidade das chaves
  ...
  df.dadosHidro <- leituraDadosUsinasHidro(pastaCaso) %>% magrittr::use_series(df.dadosUsinasHidroeletricas) %>% 
  ...
  df.volume <- dplyr::left_join(df.volume, df.dadosHidro, by = "codUsina") %>%
    dplyr::mutate(volumeMaximo = ifelse(!is.na(volumeMaximo), volumeMaximo, volumeMinDados + (volumeMaxDados - volumeMinDados) * volumeMaximoPU / 100),
  ...
  df.dadosHidro <- dplyr::left_join(df.dadosHidro, dplyr::select(df.volume, -volumeMaximoPU,-volumeMinimoPU,-dataFato), by = "codUsina") %>% 
    dplyr::mutate(volumeMaxDados = ifelse(!is.na(volumeMaximo), volumeMaximo, volumeMaxDados),
  ...
  df.volumeTempo <- tidyr::separate(df.volumeTempo, valores, into = c("mes", "ano" , "valor", "subvalor"), sep = ("\\s+"), fill = "right") %>%
    dplyr::mutate(ano = stringr::str_replace(ano, pattern = "[a-z|A-Z]+", ""), # para evitar warning de NA da transformacao para numerico
                  ano = as.numeric(ano),
  ...
  if (length(df.alteracaoConjunto$chave != 0)){
    df.alteracaoConjunto <- tidyr::separate(df.alteracaoConjunto, valores, into = c("valor", "conjunto"), sep = ("\\s+"), fill = "right") %>%
      dplyr::mutate(chave = stringr::str_replace(chave, "NUMMAQ", "numeroMaquinas"),
  ...
  df.alteracaoHidro <- dplyr::inner_join(dplyr::mutate(df.alteracaoHidro, aux = 1), dplyr::mutate(definePeriodo(pastaCaso), aux = 1), by = c("aux"), relationship = "many-to-many") %>%
    dplyr::select(-aux, -ano, -mes) %>% dplyr::mutate(vale = ifelse(((anoMes >= dataFato) | is.na(dataFato)), "s", "n")) %>%
  ...
  df.alteracaoHidro <- df.alteracaoHidro %>% dplyr::arrange(desc(dataFato)) %>% 
