...
#'
#' Faz a leitura do arquivo do NEWAVE com dados de alteracao de configuracao 
#' hidroeletrica (modif.*). Usa as funcoes internas \code{\link{leituraArquivos}},
...
#' Retorna as ateracoes no tempo (por todos os meses e anos do horizonte mesmo 
#' que o valor seja o mesmo) e sem os periodos de pre e pos
#'
#' @param pastaCaso caracter com localizacao dos arquivos NEWAVE
#'
#' @return \code{lt.alteracaoDadosUsinasHidro} lista com data frames com dados 
#' alteracao de configuracao hidroeletrica
#' \itemize{
#' \item \code{df.alteracaoHidro} data frame com dados de alteracao de 
...
#' \item volume minimo operativo no tempo [hm3] (\code{$volumeMinimo}) - como e 
#' no tempo este campo representa tanto VOLMIN e VMINT
#' \item volume minimo operativo no tempo [p.u.] do volume util 
#' (\code{$volumeMinimoPU}) - como e no tempo este campo representa tanto VOLMIN 
#' e VMINT
#' \item volume maximo operativo no tempo [hm3] (\code{$volumeMaximo}) - como e 
#' no tempo este campo representa tanto VOLMAX e VMAXT
#' \item volume maximo operativo no tempo [p.u.] do volume util 
#' (\code{$volumeMaximoPU}) - como e no tempo este campo representa tanto VOLMAX 
#' e VMAXT
#' \item numero de unidades de base (\code{$numeroUnidadeBase})
#' \item total de conjuntos de maquinas (considerados apenas os n primeiros 
#' conjuntos, de acordo com a ordem destes conjuntos no cadastro)
...
#' }
#' \item \code{df.alteracaoConjunto} data frame com dados de alteracao de 
#' configuracao por conjunto de maquinas
#' \itemize{
#' \item codigo da usina no cadastro de usinas hidroeletricas (\code{$codUsina})
#' \item anoMes no formato aaaamm (\code{$anoMes})
...
leituraAlteracaoDadosUsinasHidro <- function(pastaCaso) {
  ...
  df.alteracaoHidro <- readr::read_fwf(paste(pastaCaso, arquivo, sep = "/"),
                                       ...
                                       col_types = readr::cols(chave = readr::col_character(), 
                                                               valores = readr::col_character()),
                                       skip = 2
  )
  ...
  # preenche campos vazios com o valor anterior (da usina)
  df.alteracaoHidro <- df.alteracaoHidro %>% tidyr::fill(codUsina) 
  # retira linhas com os codigos das usinas como chave
  df.alteracaoHidro <- dplyr::filter(df.alteracaoHidro, chave != "USINA") 
  df.alteracaoHidro <- df.alteracaoHidro %>% 
    dplyr::mutate(codUsina = as.integer(codUsina))
  
  df.dadosHidro <- leituraDadosUsinasHidro(pastaCaso) %>%
    magrittr::use_series(df.dadosUsinasHidroeletricas) %>%
    dplyr::mutate(volumeMinDados = volumeMinimo, 
                  volumeMaxDados = volumeMaximo) %>%
    dplyr::select(codUsina, volumeMinDados, volumeMaxDados)
  
  # volume maximo e minimo operativos
  # estes dados serao usados como volumes maximo e minimo ate que apareca 
  # informacao em algum ponto no horizonte as atualizando (VMAXT e VMINT)
  df.volume <- df.alteracaoHidro %>% 
    dplyr::filter((chave == "VOLMAX") | (chave == "VOLMIN"))
  
  df.volume <- tidyr::separate(df.volume, 
                               valores, 
                               into = c("valor", "subvalor"), 
                               sep = ("\\s+"), 
                               fill = "right") %>%
    dplyr::mutate(
      dataFato = "",
      chave = ifelse((chave == "VOLMAX" & stringr::str_detect(subvalor, "h")), 
                     "volumeMaximo", 
                     ifelse(chave == "VOLMAX", 
                            "volumeMaximoPU", 
                            chave)),
      chave = ifelse((chave == "VOLMIN" & stringr::str_detect(subvalor, "h")), 
                     "volumeMinimo", 
                     ifelse(chave == "VOLMIN", 
                            "volumeMinimoPU", 
                            chave)),
    ...
    # em caso de multiplas alteracoes, pega a ultima
    dplyr::slice_tail() %>% 
    dplyr::ungroup() %>%
    # faz pivot para criar campos existentes em outas unidades
  ...
  df.volume <- dplyr::left_join(df.volume, df.dadosHidro, by = "codUsina") %>%
    dplyr::mutate(
      volumeMaximo = ifelse(!is.na(volumeMaximo), 
                            volumeMaximo, 
                            volumeMinDados + (volumeMaxDados - volumeMinDados) * volumeMaximoPU / 100),
      volumeMinimo = ifelse(!is.na(volumeMinimo), 
                            volumeMinimo, 
                            volumeMinDados + (volumeMaxDados - volumeMinDados) * volumeMinimoPU / 100),
      volumeMaximoPU = ifelse(!is.na(volumeMaximoPU), 
                              volumeMaximoPU, 
                              (volumeMaximo - volumeMinDados) * 100 / (volumeMaxDados - volumeMinDados)),
      volumeMinimoPU = ifelse(!is.na(volumeMinimoPU), 
                              volumeMinimoPU, 
                              (volumeMinimo - volumeMinDados) * 100 / (volumeMaxDados - volumeMinDados))
    ) %>%
    dplyr::select(-volumeMinDados, -volumeMaxDados)
  
  df.dadosHidro <- dplyr::left_join(df.dadosHidro, 
                                    dplyr::select(df.volume, -volumeMaximoPU, -volumeMinimoPU, -dataFato), 
                                    by = "codUsina") %>%
    dplyr::mutate(
      volumeMaxDados = ifelse(!is.na(volumeMaximo), 
                              volumeMaximo, 
                              volumeMaxDados),
      volumeMinDados = ifelse(!is.na(volumeMinimo), 
                              volumeMinimo, 
                              volumeMinDados)
    ) %>%
  ...
  df.volume <- df.volume %>%
    tidyr::pivot_longer(cols = c(-codUsina, -dataFato), 
                        names_to = "chave", 
                        values_to = "valor") %>%
    dplyr::filter(!is.na(valor))
  
  # volume maximo e minimo (com data)
  df.volumeTempo <- dplyr::filter(df.alteracaoHidro, ((chave == "VMINT") | 
                                                        (chave == "VMAXT")))
  df.volumeTempo <- tidyr::separate(df.volumeTempo, 
                                    valores, 
                                    into = c("mes", "ano", "valor", "subvalor"), 
                                    sep = ("\\s+"), 
                                    fill = "right") %>%
    dplyr::mutate(
      # para evitar warning de NA da transformacao para numerico
      ano = stringr::str_replace(ano, pattern = "[a-z|A-Z]+", ""), 
      ano = as.numeric(ano),
      chave = ifelse((chave == "VMAXT" & stringr::str_detect(subvalor, "h")), 
                     "volumeMaximo", 
                     ifelse(chave == "VMAXT", 
                            "volumeMaximoPU",
                            chave)),
      chave = ifelse((chave == "VMINT" & stringr::str_detect(subvalor, "h")), 
                     "volumeMinimo", 
                     ifelse(chave == "VMINT", 
                            "volumeMinimoPU", 
                            chave)),
  ...
  # calcula campos a partir dos que existem
  df.volumeTempo <- dplyr::left_join(df.volumeTempo, 
                                     df.dadosHidro, 
                                     by = "codUsina") %>%
    dplyr::mutate(
      volumeMaximo = ifelse(!is.na(volumeMaximo), 
  ...
  df.dadosTempo <- dplyr::filter(df.alteracaoHidro, ((chave == "CFUGA") | 
                                                       (chave == "CMONT") | 
                                                       (chave == "VAZMINT") | 
                                                       (chave == "VAZMAXT")))
  df.dadosTempo <- tidyr::separate(df.dadosTempo, 
                                   valores, 
                                   into = c("mes", "ano", "valor"), 
                                   sep = ("\\s+"), 
                                   fill = "right") %>%
    dplyr::mutate(
      # para evitar warning de NA da transformacao para numerico
  ...
  # numero de unidades de base, total de conjuntos de maquinas, 
  # vazao maxima e minima
  df.dadosValorUnico <- dplyr::filter(df.alteracaoHidro, ((chave == "NUMBAS") | 
                                                            (chave == "NUMCNJ") | 
                                                            (chave == "VAZMIN") | 
                                                            (chave == "VAZMAX") |
                                                            (chave == "PRODESP") | 
  ...
  df.alteracaoConjunto <- dplyr::filter(df.alteracaoHidro, ((chave == "NUMMAQ") | 
                                                              (chave == "POTEFE")))
  
  # pode ocorrer de nao haver este tipo de registro no arquivo modif
  if (length(df.alteracaoConjunto$chave != 0)) {
    df.alteracaoConjunto <- tidyr::separate(df.alteracaoConjunto, 
                                            valores, into = c("valor", "conjunto"), 
                                            sep = ("\\s+"), 
                                            fill = "right") %>%
      dplyr::mutate(
      ...
      dplyr::ungroup() %>%
      tidyr::pivot_wider(names_from = chave, values_from = valor)
    df.alteracaoConjunto <- dplyr::inner_join(dplyr::mutate(df.alteracaoConjunto, aux = 1), 
                                              dplyr::mutate(definePeriodo(pastaCaso), aux = 1), 
                                              by = c("aux"), 
    ...
    df.alteracaoConjunto <- data.frame(codUsina = 999, 
                                       anoMes = 999999, 
                                       conjunto = 99, 
                                       numeroMaquinas = NA, 
                                       potenciaEfetiva = NA)
  ...
  df.alteracaoHidro <- rbind(df.volume, 
                             df.volumeTempo, 
                             df.dadosTempo, 
                             df.dadosValorUnico)
  
  # coloca dados ao longo do tempo restringindo a existencia de dados anteriores 
  # as datas de modificacao
  df.alteracaoHidro <- dplyr::inner_join(dplyr::mutate(df.alteracaoHidro, aux = 1), 
                                         dplyr::mutate(definePeriodo(pastaCaso), aux = 1), 
                                         by = c("aux"), 
                                         relationship = "many-to-many") %>%
  ...
  # elimina os dados duplicados das modificacoes, ficando somente a correta 
  ...
  df.alteracaoHidro <- df.alteracaoHidro %>% dplyr::select(
    codUsina, anoMes, canalFuga, nivelMontante, vazaoMinima, vazaoMaxima, 
    volumeMinimo, volumeMinimoPU, volumeMaximo, volumeMaximoPU, 
    numeroUnidadeBase, numeroConjuntos, produtibilidade, TEIF, IP, perda
  )
  
  lt.alteracaoDadosUsinasHidro <- list(df.alteracaoHidro = df.alteracaoHidro, 
                                       df.alteracaoConjunto = df.alteracaoConjunto)
